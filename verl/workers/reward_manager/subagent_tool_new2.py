...
from verl.workers.reward_manager.tool import (
    ...
    run_async_computation2,
)


def nearest_resample(x, target_len, response_str=None):
    """
    Nearest-neighbor resampling of a 1D list to a fixed length.

    Args:
        x (list or np.ndarray): input sequence, length n
        target_len (int): desired output length L

    Returns:
        np.ndarray: resampled sequence of length target_len
    """
    x = np.asarray(x)
    n = len(x)
    ...
    if n == 0:
        ...
        return np.array([response_str] * target_len)
    if n == 1:
    ...
    ## src_pos = np.linspace(0, 1, n)
    tgt_pos = np.linspace(0, 1, target_len)

    # nearest neighbor index mapping
    indices = np.round(tgt_pos * (n - 1)).astype(int)
...
class NewSubagentToolRewardManager2(ToolRewardManager):
    ...
    def _call_include_subagent_tool(self, data: DataProto, return_dict=False):
        ...
        main_agent_split_response_str_list = []
        ...
        for i in range(len(prompt_str_list)):
            if not(is_from_subagent_tool_list[i]):
                ...
                if not(len(response_str_list_new[parent_idx_list == i]) == len(summary_stage_this)):
                    ...
                    length_target = len(response_str_list_new[parent_idx_list == i])
